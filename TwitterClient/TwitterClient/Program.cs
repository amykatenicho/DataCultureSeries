    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            var datum = Tweet.StreamStatuses(new TwitterConfig(oauthToken, oauthTokenSecret, oauthCustomerKey, oauthConsumerSecret,
                keywords)).Select(tweet => Sentiment.ComputeScore(tweet, keywords)).Select(tweet => new Payload { CreatedAt=tweet.CreatedAt, UserName =tweet.UserName,Topic =tweet.Topic ,SentimentScore =tweet.SentimentScore, Text = tweet.Text, Location = tweet.Location });
